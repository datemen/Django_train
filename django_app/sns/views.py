...
def groups(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            vlist = []
            for item in sel_users:
                new_friends = Friend()
                new_friends.owner = request.user
                new_friends.user = item
                new_friends.group = group_obj
                new_friends.save()
...
def add(request):
    ...
    if add_user == request.user:
        messages.info(request, "自分自身をFriendに追加することができません。")
...
def post(request):
    ...
    if request.method == 'POST':
        ...
        msg.file = form.cleaned_data['file']
        msg.group = group
        msg.content = content
        msg.save()
        #メッセージを設定
...
#これ以降はビュー関数ではなくて普通の関数

#指定されたグループ及び検索文字によりMessageの取得
...
def friends(request):
    ...
    if request.method == 'GET':
        ...
        account_user = User.objects.all()
        
        friends_now = Friend.objects.all()
        
        #共通処理
        params = {
                'login_user':request.user,
                'login_user_name':request.user.username,
                'contents':account_user,
                'friends':friends_now,
            }
    return render(request, 'sns/friends.html', params)
...
def photo(request):
    ...
    return render(request, 'sns/photo.html', params)
'''